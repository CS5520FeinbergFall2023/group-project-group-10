...
public class FriendRequestAdapter extends RecyclerView.Adapter<FriendRequestViewHolder> {
    ...
    private void denyRequest(String fromUid, int position) {
        ...
        /*
        frRef.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot frSnapshot: snapshot.getChildren()) {
                    frSnapshot.getRef().removeValue();
                }
                DatabaseReference othersFriendRef = db.getReference("users").child(fromUid).child("friends").child(currUser.getUid());
                othersFriendRef.removeValue();
            }
    ...
    // delete the request
    // add user if approved to friends and update the other user's friends status to friends
    // delete user from other user's friends if denied
